...
}, b = slider(min = 5, max = 50, initial = 25, label = 'bin adjustment', step = 5))
```
...
```{r ggvis input_slider, echo=FALSE}
library(ggvis) 
...
cars %>% 
  ggvis(x = ~wt, y = ~mpg, key := ~id) %>%
  layer_points() %>%
  add_tooltip(all_values, "click") ##now you have to click the points 
```
...
```{r task: datatable}
datatable(iris, options = list(pageLength=5, dom='tp'))
